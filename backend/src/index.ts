...
app.get('/schedule/:shareToken', async (req, res) => {
  try {
    ...
    if (restaurant) {
      ...
      const host = req.headers.host || 'bistroflow.com';
      const origin = `${protocol}://${host}`;
      const url = `${origin}/schedule/${req.params.shareToken}`;
      const logoUrl = restaurant.logoUrl
        ? `${origin}${restaurant.logoUrl}`
        : `${origin}/logo.png`;
    ...
    <meta property="og:site_name" content="BistroFlow" />
    ...
    <meta name="twitter:image" content="${logoUrl}" />`;
      ...
      html = html.replace('<title>BistroFlow</title>', `<title>${title}</title>`);
      html = html.replace(
        '<meta name="description" content="BistroFlow - Premium Restaurant Operations & Management Platform" />',
