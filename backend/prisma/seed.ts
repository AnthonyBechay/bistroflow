...
async function main() {
  ...
  // Create Shifts for Bistro Grande Employees
  ...
  const storageDry = await prisma.storageLocation.create({
    data: { name: 'Dry Pantry', notes: 'Main pantry shelves', userId },
  });
  const storageWalkIn = await prisma.storageLocation.create({
  ...
  const ingSubGrains = await prisma.ingredientSubcategory.create({
